  ...
  useEffect(() => {
    if (dataProductAssetsStatus === 'succeeded') {
      ...
      const assetData: any[] = dataProductAssets.map((asset: any) => {
        ...
        const resourceParts = asset.resource.split('/');
        const projectsPart = asset.resource.split('projects/')[1];
        const sourceProject = projectsPart ? projectsPart.split('/')[0] : 'Unknown';

        return ({
          Name: resourceParts.pop() || 'Unknown',
          Type: resourceParts.length > 0 ? resourceParts.pop() : 'Unknown',
          System: asset.resource.includes('//') ? 'External' : 'Bigquery',
