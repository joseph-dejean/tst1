...
const DataProductOverviewNew: React.FC<DataProductOverviewNewProps> = ({ entry, entryType, sampleTableData, css, }) => {
    ...
    const type = entryType || entry.entryType || '';
    ...
    let schemaData = entry.aspects?.[`${typePrefix}.global.schema`]?.data?.fields?.fields?.listValue?.values || [];
    let contacts = entry.aspects?.[`${typePrefix}.global.contacts`]?.data?.fields?.identities?.listValue?.values || [];
    let usage = entry.aspects?.[`${typePrefix}.global.usage`]?.data?.fields || {};
    let documentation = entry.aspects?.[`${typePrefix}.global.overview`]?.data?.fields?.content?.stringValue || 'No Documentation Available';

    if (entryType && entryType == 'data-product') {
        contacts = entry.aspects?.[`${typePrefix}.global.contacts`]?.data?.identities || [];
        console.log("contact DATA - ", contacts);
        documentation = entry.aspects?.[`${typePrefix}.global.overview`]?.data?.content || 'No Documentation Available';
    }
